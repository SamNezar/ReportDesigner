...
class RDLCExporter {
    ...
    //          paramEl.appendChild(this.createElement('Nullable')).textContent = 'true';
    //          paramEl.appendChild(this.createElement('AllowBlank')).textContent = 'true';
    ...
    // [REPLACE THIS FUNCTION in RDLCExporter.js]
_createParametersElement() {
    // Use a Map to store the full parameter definition for easy lookup.
    const paramDefinitions = new Map(this.designer.parameters.map(p => [p.name, p]));

    // Use a Set to collect all unique parameter names to avoid duplicates.
    const allParamNames = new Set(this.designer.parameters.map(p => p.name));
    
    // Regex to find all instances of '=Parameters!PARAM.Value'
    const paramRegex = /Parameters!(\w+)\.Value/g;
    ...
    // Scan all report objects for parameter usage.
    this.designer.objects.forEach(obj => {
        if (obj.type === 'textbox') {
            const plainText = getPlainText(obj.text);
            let match;
            while ((match = paramRegex.exec(plainText)) !== null) {
                allParamNames.add(match[1]); // Add the captured parameter name
            }
        } else if (obj.type === 'table') {
            obj.rows.forEach(row => {
                row.cells.forEach(cell => {
                    const plainText = getPlainText(cell.text);
                    let match;
                    while ((match = paramRegex.exec(plainText)) !== null) {
    ...
    if (allParamNames.size === 0) return null;

    const reportParametersEl = this.createElement('ReportParameters');

    allParamNames.forEach(name => {
        // Check if the parameter was pre-defined or found dynamically.
        const existingParam = paramDefinitions.get(name);
        
        const paramEl = this.createElement('ReportParameter');
        paramEl.setAttribute('Name', name);

        // Use existing definition or create a default one for dynamically found params.
        const dataType = existingParam ? (existingParam.type || 'String') : 'String';
        const prompt = existingParam ? (existingParam.prompt || name) : name;

        paramEl.appendChild(this.createElement('DataType')).textContent = dataType;
        paramEl.appendChild(this.createElement('Nullable')).textContent = 'true';
        paramEl.appendChild(this.createElement('AllowBlank')).textContent = 'true';
        paramEl.appendChild(this.createElement('Prompt')).textContent = prompt;
        ...
        if (isExpression) {
            ...
            if (!expressionAdded) {
                ...
                const styleEl = this.createElement('Style');
                 ...
                 if (textbox.format) {
                styleEl.appendChild(this.createElement('Format')).textContent = textbox.format;
            }
...
// Export image object - FIXED for External Sources
exportImage(image, index, image_index) {
    const imageEl = this.createElement('Image');
    imageEl.setAttribute('Name', `Image${index + 1}`);

    // Determine the correct source type by inspecting the image.src property.
    // Embedded images will have a base64 data URL, which starts with "data:".
    const isEmbedded = image.src && image.src.startsWith('data:');

    const sourceEl = this.createElement('Source');
    sourceEl.textContent = isEmbedded ? 'Embedded' : 'External';
    imageEl.appendChild(sourceEl);

    const valueEl = this.createElement('Value');
    if (isEmbedded) {
        // For embedded images, the value is the name of the resource in the <EmbeddedImages> collection.
        valueEl.textContent = `EmbeddedImage${image_index + 1}`;
    } else {
        // For external images, the value is the URL itself.
        // If image.src is null or empty (e.g., from an expression in the original RDLC),
        // this will correctly output an empty <Value> tag.
        // Note: The original importImage function discards expressions, so we cannot restore them here.
        valueEl.textContent = image.src || '';
    }
    ...
    // The imageData is only returned for embedded images to be added to the collection.
    return { imageElement: imageEl, imageData: isEmbedded ? image.src : null };
}
    // Export image object
    // exportImage(image, index, image_index) {
    //     const imageEl = this.createElement('Image');
    ...
    //     //if (image.src) {
    //     const valueEl = this.createElement('Value');
    //     valueEl.textContent = image.src ? `EmbeddedImage${image_index + 1}` : `null`;
    //     imageEl.appendChild(valueEl);
    //     //}
...
exportTable(table, index) {
                    ...
                    if (isExpression) {
                        if (!expressionAdded) {
                            ...
                            const styleEl = this.createElement('Style');
                              ...
                              if (cellTextbox.format) {
            styleEl.appendChild(this.createElement('Format')).textContent = cellTextbox.format;
        }
    ...
    tablixEl.appendChild(tablixRowHierarchyEl);
     ...
     // Add the DataSetName element, taken from the first available dataset.
    if (this.designer.datasets && this.designer.datasets.length > 0) {
        const dataSetNameEl = this.createElement('DataSetName');
        dataSetNameEl.textContent = this.designer.datasets[0].name;
        tablixEl.appendChild(dataSetNameEl);
    }
   ...
   // Main export function - UPDATED for Header/Footer support
export() {
    this.designer.updateObjectSections();
    ...
    // --- Segregate objects by section ---
    const headerObjects = this.designer.objects.filter(obj => obj.section === 'header');
    const footerObjects = this.designer.objects.filter(obj => obj.section === 'footer');
    const bodyObjects = this.designer.objects.filter(obj => obj.section === 'body' || !obj.section); // Default to body
    ...
    // --- Helper function to process a list of objects and create a ReportItems element ---
    const processObjects = (objectList, yOffset = 0) => {
        if (!objectList || objectList.length === 0) return null;

        const reportItemsEl = this.createElement('ReportItems');
        objectList.forEach(obj => {
            // Create a temporary object with a y-coordinate relative to its section
            const tempObj = { ...obj };
            tempObj.y -= yOffset;

            switch (tempObj.type) {
                case 'textbox':
                    reportItemsEl.appendChild(this.exportTextbox(tempObj, objectIndex++));
                    break;
                case 'image':
                    const { imageElement, imageData } = this.exportImage(tempObj, objectIndex++, embeddedImages.length);
                    ...
                    break;
                case 'table':
                    reportItemsEl.appendChild(this.exportTable(tempObj, objectIndex++));
                    break;
                case 'line':
                    reportItemsEl.appendChild(this.exportLine(tempObj, objectIndex++));
                    break;
            }
        });
        return reportItemsEl;
    };

    // --- 1. Process Body ---
    const bodyEl = this.createElement('Body');
    const bodyReportItemsEl = processObjects(bodyObjects, this.designer.headerHeight);
    if (bodyReportItemsEl) {
        bodyEl.appendChild(bodyReportItemsEl);
    }
    const bodyHeight = pageHeightPx - (this.designer.headerVisible ? this.designer.headerHeight : 0) - (this.designer.footerVisible ? this.designer.footerHeight : 0);
    bodyEl.appendChild(this.createElement('Height')).textContent = this.pxToInches(bodyHeight > 0 ? bodyHeight : pageHeightPx);
    bodyEl.appendChild(this.createElement('Style'));
    reportSectionEl.appendChild(bodyEl);

    // --- 2. Process Page (including Header and Footer) ---
    ...
    // --- 2a. Page Header ---
    if (this.designer.headerVisible) {
        const pageHeaderEl = this.createElement('PageHeader');
        pageHeaderEl.appendChild(this.createElement('Height')).textContent = this.pxToInches(this.designer.headerHeight);
        pageHeaderEl.appendChild(this.createElement('PrintOnFirstPage')).textContent = 'true';
        pageHeaderEl.appendChild(this.createElement('PrintOnLastPage')).textContent = 'true';
        
        const headerReportItemsEl = processObjects(headerObjects, 0); // No Y-offset needed for header
        if (headerReportItemsEl) {
            pageHeaderEl.appendChild(headerReportItemsEl);
        }
        
        pageHeaderEl.appendChild(this.createElement('Style'));
        pageEl.appendChild(pageHeaderEl);
    }

    // --- 2b. Page Footer ---
    if (this.designer.footerVisible) {
        const pageFooterEl = this.createElement('PageFooter');
        pageFooterEl.appendChild(this.createElement('Height')).textContent = this.pxToInches(this.designer.footerHeight);
        pageFooterEl.appendChild(this.createElement('PrintOnFirstPage')).textContent = 'true';
        pageFooterEl.appendChild(this.createElement('PrintOnLastPage')).textContent = 'true';
        
        const footerYOffset = pageHeightPx - this.designer.footerHeight;
        const footerReportItemsEl = processObjects(footerObjects, footerYOffset);
        if (footerReportItemsEl) {
            pageFooterEl.appendChild(footerReportItemsEl);
        }

        pageFooterEl.appendChild(this.createElement('Style'));
        pageEl.appendChild(pageFooterEl);
    }
    ...
    ['Left', 'Right', 'Top', 'Bottom'].forEach(side => {
        pageEl.appendChild(this.createElement(`${side}Margin`)).textContent = '0in';
    });
