// RDLC Import Module for Report Designer - v2 (Corrected)
class RDLCImporter {
    ...
    import(xmlString) {
        ...
        this.importPageSettings();

        const processItems = (itemNodes) => {
            itemNodes.forEach(node => {
                ...
                if (newObject) {
                    this.designer.addObject(newObject);
                      ...
                      newObject.applyStyles({
                    padding: newObject.padding,
                });
        ...
        // Import items from Body and PageHeader
        processItems(this.xpath_query('//def:Body/def:ReportItems/*'));
        processItems(this.xpath_query('//def:PageHeader/def:ReportItems/*'));

             // ADDED: Final step to apply z-index and reset the designer's counter
        let maxZ = 0;
   ...
   image.zIndex = parseInt(this.getNodeValue(node, 'def:ZIndex', '0'));
        ...
        if (source === 'Embedded' && value) {
            ...
            if (embeddedImageNode) {
             ...
             image.src = value.startsWith('=') ? null : value; // Cannot resolve expressions
        }
  ...
  //      const padding = this.unitToPx(this.getNodeValue(styleNode, 'def:PaddingLeft', `${obj.padding}px`));
//        obj.padding = padding;
    ...
    parseParagraphsToHtml(paragraphsNode) {
        ...
        paragraphNodes.forEach(pNode => {
            ...
            textRunNodes.forEach(trNode => {
                ...
                // Handle expressions by showing the expression itself
                // if (value.startsWith('=')) {
                //     value = value.replace(/=Parameters!(\w+)\.Value/, '[$1]')
                //                  .replace(/=Fields!(\w+)\.Value/, '[$1]')
                //                  .replace(/=Sum\((.*?)\)/, '[Sum($1)]')
                //                  .replace(/=RowNumber\(nothing\)/i, '[#]');
                // }
