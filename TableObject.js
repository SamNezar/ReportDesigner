class TableObject extends ReportObject {
  ...
  // --- REPLACE this method in TableObject.js with the new version below ---
    showFieldPickerMenu(event, textboxObject) {
       ...
       // Check if there are any fields with for_table === true
        let hasTableFields = false;
        ...
        if (!hasTableFields) {
            ...
            noDataItem.textContent = 'No table fields available';
            ...
            reportDesigner.datasets.forEach(dataset => {
                dataset.fields.forEach(field => {
                    if (field.for_table) {
                        ...
                        fieldItem.textContent = field.displayName;
