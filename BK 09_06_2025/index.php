    ...
    <script>
        ...
        class TextBox extends ReportObject {
            ...
            createElement() {
                ...
                // Show «Expr» only for complex expressions.
                // For simple values or literals, show the content generated by the importer (e.g., [field_name]).
                ...
                contentDiv.addEventListener('blur', () => {
                    ...
                    if (textType === 'expression') {
                        // Also use the new method here to preserve formatting on blur.
                        contentDiv.innerHTML = this.createFormattedPlaceholder(this.text, '«Expr»');
                    }
                });
            ...
            /**
             * Classifies text content as a literal, a simple value reference, or a complex expression.
             * @param {string} text - The plain text content to analyze.
             * @returns {'literal' | 'value' | 'expression'}
             */
            classifyText(text) {
                const trimmedText = text.trim();
                if (!trimmedText.startsWith('=')) {
                    return 'literal'; // It's just plain text.
                }

                // Regex to match ONLY a simple field or parameter reference.
                const simpleValueRegex = /^=(Fields|Parameters)!(\w+)\.Value$/;

                if (simpleValueRegex.test(trimmedText)) {
                    // It's a direct reference to a single value.
                    return 'value';
                } else {
                    // It starts with '=' but is more complex.
                    return 'expression';
                }
            }
            ...
            init() {
                ...
                // --- Method to add a DataSet ---
                    addDataSet(name, fields) {
                        ...
                        const correctedFields = fields.map(f => ({...f, visible: f.visible !== false}));
                        this.datasets.push({ name, fields: correctedFields });
                    }
                    ...
                    addParameters(parameters) {
                        ...
                        // Fix the 'visible' property assignment
                        this.parameters = parameters.map(p => ({...p, visible: p.visible !== false}));
                    }
