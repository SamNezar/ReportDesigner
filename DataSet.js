    ...
    // --- Data Type Icons ---
        const dataTypeIcons = {
            ...
            'Float': '💲',            // Decimal (for parameters)
            'System.Boolean': '☑️',   // Boolean
            'Boolean': '☑️',          // Boolean (for parameters)
            'System.Int32': '#️⃣',    // Integer
            'Int32': '#️⃣',           // Integer (for parameters)
            'System.DateTime': '📅',  // DateTime
            'DateTime': '📅',         // DateTime (for parameters)
            'Date': '📅'              // Date (if used as custom type or in frontend)
        };
        ...
        if (reportDesigner.datasets && reportDesigner.datasets.length > 0) {
            reportDesigner.datasets.forEach(dataset => {
                ...
                dataset.fields.forEach(field => {
                    if (field.visible && !field.for_table) {
                        data_found = true;
...
//         { name: 'transaction_item_name', type: 'System.String', displayName: 'اسم الصنف', displayValue: '[transaction_item_name]', visible: true, for_table:true },
//         { name: 'transaction_qty', type: 'System.Decimal', displayName: 'Item QTY', displayValue: '[transaction_qty]', visible: true, for_table:true },
//         { name: 'transaction_unit_price', type: 'System.Decimal', displayName: 'Unit Price', displayValue: '[transaction_unit_price]', visible: false, for_table:true },
//     ]);
